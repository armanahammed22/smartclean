        ...
        <div className="lg:col-span-5 space-y-6">
          <Card className="border-none shadow-sm bg-white rounded-3xl overflow-hidden">
            ...
            <CardContent className="p-8 space-y-6">
              ...
              <div className="space-y-2">
                <Label className="text-[10px] font-black uppercase text-muted-foreground ml-1">Ending Date & Time</Label>
                ...
                <p className="text-[9px] text-muted-foreground italic px-1">Flash sale will auto-hide from homepage after this time.</p>
              </div>

              <div className="pt-6 border-t border-gray-100 flex flex-col gap-4">
                <div className="flex items-center justify-between">
                  <span className="text-xs font-bold text-gray-600">Selected Products</span>
                  <Badge className="bg-primary text-white font-black">{formData.productIds.length}</Badge>
                </div>
                <div className="flex flex-wrap gap-2">
                  {formData.productIds.map(pid => (
                    <Badge key={pid} variant="secondary" className="bg-gray-100 text-[10px] font-bold py-1 pl-3 pr-1 gap-2 rounded-lg">
                      {allProducts?.find(p => p.id === pid)?.name || pid}
                      <button onClick={() => toggleProduct(pid)} className="p-0.5 hover:bg-gray-200 rounded-full"><Trash2 size={10}/></button>
                    </Badge>
                  ))}
                </div>
              </div>
        ...
        <div className="lg:col-span-7">
          <Card className="border-none shadow-sm bg-white rounded-3xl overflow-hidden border border-gray-100 h-[700px] flex flex-col">
            <CardHeader className="bg-gray-50/50 border-b p-6">
              <CardTitle className="text-sm font-black uppercase tracking-widest text-muted-foreground">Inventory Selection</CardTitle>
            </CardHeader>
                ...
                {allProducts?.map((product) => (
                  <div 
                    ...
                    className={cn(
                      ...
                      formData.productIds.includes(product.id) ? "border-primary bg-primary/5" : "border-gray-50 bg-white hover:border-gray-200"
                    )}
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-[10px] font-bold text-primary mt-0.5">৳{product.price.toLocaleString()}</p>
                    </div>
                    <div className={cn(
                      "w-6 h-6 rounded-full border-2 flex items-center justify-center transition-all",
                      formData.productIds.includes(product.id) ? "bg-primary border-primary text-white" : "border-gray-200"
                    )}>
                      {formData.productIds.includes(product.id) && <Plus size={14} strokeWidth={4} />}
                    </div>
                  </div>
                ))}
              </div>
