      ...
      <div className="container mx-auto px-4 flex flex-col items-center">
        ...
        <div className="w-full max-w-[210mm] flex flex-col sm:flex-row justify-between items-center mb-10 gap-6 px-4">
          ...
          <div className="flex gap-3">
             <Button 
                className="rounded-xl gap-2 font-black uppercase text-[10px] h-12 px-10 bg-[#1E5F7A] shadow-xl shadow-primary/20 hover:scale-105 transition-all"
        ...
        >
          ...
          <div className="absolute top-0 right-0 p-10 flex flex-col items-end gap-1.5 z-10 text-[9px] font-black text-[#1E5F7A] uppercase tracking-widest">
            ...
            <div className="flex items-center gap-2 bg-gray-50/80 backdrop-blur-md px-3 py-1.5 rounded-lg border border-primary/10 shadow-sm"><Globe size={11}/> {settings?.websiteUrl?.replace('https://', '')}</div>
          </div>
          ...
          <div className="pt-10 px-16 relative">
             <div className="flex items-start gap-5 mb-12">
                ...
                <div className="pt-2">
                  ...
                  <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mt-2">Certified Professional Services</p>
                </div>
             </div>
          ...
          <div className="px-16 pt-10 space-y-12 relative z-10">
             <div className="flex justify-between items-start">
                ...
                <div className="text-right space-y-6">
                  <div className="space-y-1">
                    <p className="text-[10px] font-black text-[#1E5F7A] uppercase tracking-widest">Invoice Reference</p>
                    <p className="text-lg font-black text-[#081621] font-mono tracking-tight">{invoice.invoiceNumber}</p>
                  </div>
                  <div className="space-y-1">
                    <p className="text-[10px] font-black text-[#1E5F7A] uppercase tracking-widest">Date of Bill</p>
                ...
                <table className="w-full border-collapse">
                  <thead className="bg-[#00A8B5] text-white">
                    <tr>
                      ...
                      <th className="py-4 px-4 text-[10px] font-black border-r border-[#081621] uppercase text-left">Description of Services</th>
                  ...
                  <tbody className="text-[11px] font-medium bg-white">
                    {invoice.items.map((item: any, i: number) => (
                      <tr key={i} className="border-t border-[#081621]">
                        ...
                        <td className="py-4 text-center border-r border-[#081621] font-black text-gray-700">
                          {item.quantity} <span className="text-[8px] font-bold text-gray-400 ml-0.5 uppercase">{item.unit || 'Qty'}</span>
                        </td>
                        <td className="py-4 text-center border-r border-[#081621] font-black text-gray-700">
                          {item.price?.toLocaleString()}
                        </td>
                        <td className="py-4 text-center font-black text-gray-900 bg-gray-50/20">
                    ...
                    {invoice.deliveryCharge > 0 && (
                      <tr className="border-t border-[#081621] bg-white">
                        <td colSpan={4} className="py-3 px-6 text-right font-black uppercase text-[10px] border-r border-[#081621]">Delivery / Extra (+)</td>
                        <td className="py-3 px-4 text-center font-black text-sm">৳{invoice.deliveryCharge.toLocaleString()}</td>
                      </tr>
                    )}
                    <tr className="border-t border-[#081621] bg-white">
                      <td colSpan={4} className="py-3 px-6 text-right font-black uppercase text-[10px] border-r border-[#081621]">Govt VAT / Tax (0%)</td>
                      <td className="py-3 px-4 text-center font-black text-sm">৳0 /-</td>
                    </tr>
             ...
             <div className="p-6 bg-[#1E5F7A]/5 rounded-2xl border-2 border-[#081621] space-y-1">
                <p className="text-[9px] font-black uppercase text-gray-400 tracking-[0.2em]">Total Amount In Words:</p>
             ...
             <div className="grid grid-cols-2 gap-12 pt-4">
                <div className="space-y-6">
                  <div className="space-y-3">
                    <h4 className="text-[10px] font-black uppercase text-[#1E5F7A] tracking-widest border-b border-dashed border-[#1E5F7A] w-fit mb-2">Billing Details:</h4>
                  ...
                  <div className="p-4 bg-blue-50/50 rounded-xl border border-blue-100 flex items-start gap-3">
                    <Info size={14} className="text-[#1E5F7A] mt-0.5 shrink-0" />
                    <p className="text-[8px] font-bold text-blue-900 leading-tight uppercase">
                        Terms: This invoice is system generated. Water and electricity must be provided at the site. For support, call 01919640422.
                    </p>
                  </div>
                </div>
                ...
                <div className="flex flex-col items-center justify-end text-center space-y-5 pb-4">
                   <div className="h-16 w-32 relative border-b-2 border-gray-100 pb-2 flex items-center justify-center">
                      {signatureUrl ? (
                        <Image src={signatureUrl} alt="Official Sign" fill className="object-contain" unoptimized />
                      ) : (
                   ...
                   <div className="space-y-0.5">
                      <p className="font-black text-xs uppercase text-gray-900 tracking-tighter">Chief Operational Officer</p>
          ...
          <div className="absolute bottom-0 left-0 right-0 h-20 border-t-4 border-[#1E5F7A] px-16 flex items-center justify-between bg-white">
             <div className="flex items-center gap-4">
                ...
                <div>
                   <p className="text-[10px] font-black text-gray-800 uppercase leading-none mb-1">Business Hub</p>
                   <p className="text-[9px] font-bold text-gray-500 uppercase leading-none">{settings?.address || 'GP.Ja-66/2, Wireless Gate, Mohakhali, Dhaka-1212'}</p>
                </div>
             </div>
        ...
        <div className="w-full max-w-[210mm] mt-16 grid grid-cols-1 md:grid-cols-3 gap-6">
            <Card className="border-none shadow-xl bg-white rounded-3xl p-6 flex items-center gap-4 hover:scale-105 transition-transform cursor-pointer group">
                ...
                <div><p className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Direct Helpline</p><p className="font-black text-sm">{settings?.contactPhone}</p></div>
            </Card>
            <Card className="border-none shadow-xl bg-white rounded-3xl p-6 flex items-center gap-4 hover:scale-105 transition-transform cursor-pointer group">
                ...
                <div><p className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Trusted Brand</p><p className="font-black text-sm">Govt. Certified Pro</p></div>
            </Card>
            <Card className="border-none shadow-xl bg-[#081621] text-white rounded-3xl p-6 flex items-center gap-4 hover:scale-105 transition-transform cursor-pointer group">
                ...
                <div><p className="text-[10px] font-black text-white/30 uppercase tracking-widest">Official Site</p><p className="font-black text-sm">smartclean.com.bd</p></div>
            </Card>
        </div>
      </div>
    </div>
