...
export default function DocumentEnginePage() {
  ...
  const [invoiceForm, setInvoiceForm] = useState<any>({
    ...
    invoiceFooterNote: '',
    invoiceFooterDisclaimer: 'This is a computer generated document and does not require a physical stamp.',
  ...
  const providedServicesPreview = (invoiceForm.invoiceProvidedServices || 'Home Cleaning, Office Cleaning, Deep Cleaning, Sofa & Carpet, Kitchen Sanitization, Pest Control').split(',').map((s: string) => s.trim()).filter((s: string) => s);
    ...
    <div className="space-y-8 pb-24 min-w-0">
      ...
      <Tabs defaultValue="design" className="w-full space-y-8">
        <TabsList className="bg-white border p-1 h-14 rounded-2xl w-full max-w-4xl shadow-sm flex">
          ...
          <TabsTrigger value="quotation" className="flex-1 rounded-xl gap-2 text-[11px] font-black uppercase data-[state=active]:bg-primary data-[state=active]:text-white transition-all"><FileSpreadsheet size={16}/> Quotation Text</TabsTrigger>
          <TabsTrigger value="invoice" className="flex-1 rounded-xl gap-2 text-[11px] font-black uppercase data-[state=active]:bg-primary data-[state=active]:text-white transition-all"><Printer size={16}/> Invoice Text</TabsTrigger>
        </TabsList>
               ...
               <Card className="border-none shadow-sm rounded-3xl overflow-hidden bg-white border border-gray-100">
                  <CardHeader className="bg-gray-50/50 p-6 border-b">
                    <CardTitle className="text-xs font-black uppercase flex items-center gap-2 text-[#081621]">
                      <Zap size={14} className="text-primary"/> Status Seals
                    </CardTitle>
                  </CardHeader>
                  ...
                  >
                    ...
                    <footer className="pt-6 border-t border-gray-100 px-12 shrink-0" style={{ marginTop: `${designForm.footerMarginTop}px`, paddingBottom: `${designForm.footerPaddingBottom}px` }}>
                       ...
                       <div className="grid grid-cols-3 gap-x-6 gap-y-0.5 opacity-60">
                          {providedServicesPreview.slice(0, 6).map((service, i) => (
                            <div key={i} className="flex items-center gap-2">
                               <CheckCircle2 size={10} className="text-emerald-500 shrink-0" />
                               <span className="text-[8px] font-bold text-gray-600 uppercase truncate">{service}</span>
                            </div>
                          ))}
                       </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
            <div className="lg:col-span-8 space-y-6">
              <Card className="border-none shadow-sm bg-white rounded-3xl overflow-hidden border border-gray-100">
                <CardHeader className="bg-[#081621] text-white p-8">
                   <CardTitle className="text-lg font-black uppercase flex items-center gap-3"><FileSpreadsheet size={20} className="text-primary" /> Header & Sequence</CardTitle>
                </CardHeader>
                <CardContent className="p-8 space-y-6">
                   <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div className="space-y-1"><Label className="text-[9px] font-black uppercase">Ref Prefix</Label><Input value={quoteForm.prefix} onChange={e => setQuoteForm({...quoteForm, prefix: e.target.value})} className="h-11 bg-gray-50 border-none rounded-xl font-bold" /></div>
                      <div className="space-y-1"><Label className="text-[9px] font-black uppercase">Start Seq.</Label><Input type="number" value={quoteForm.lastNumber} onChange={e => setQuoteForm({...quoteForm, lastNumber: parseInt(e.target.value) || 0})} className="h-11 bg-gray-50 border-none rounded-xl font-black" /></div>
                   </div>
                   <div className="space-y-1"><Label className="text-[9px] font-black uppercase">Quotation Tagline</Label><Input value={quoteForm.tagline} onChange={e => setQuoteForm({...quoteForm, tagline: e.target.value})} className="h-11 bg-gray-50 border-none rounded-xl font-bold italic" /></div>
                   <div className="space-y-1"><Label className="text-[9px] font-black uppercase">Provided Services (Footer List - Comma Separated)</Label><Textarea value={quoteForm.defaultFooterServices} onChange={e => setQuoteForm({...quoteForm, defaultFooterServices: e.target.value})} placeholder="e.g. Sofa Cleaning, Carpet Cleaning..." className="h-20 bg-gray-50 border-none rounded-xl" /></div>
                </CardContent>
              </Card>

              <Card className="border-none shadow-sm bg-white rounded-3xl overflow-hidden border border-gray-100">
                <CardHeader className="bg-gray-50/50 p-8 border-b flex flex-row items-center justify-between">
                   <CardTitle className="text-base font-black uppercase tracking-widest text-[#081621] flex items-center gap-3"><ListChecks className="text-primary"/> Default Quotation Terms</CardTitle>
                   <Button type="button" onClick={addQuoteTerm} variant="outline" size="sm" className="rounded-xl font-black text-[9px] uppercase border-primary/20 text-primary">+ Add Row</Button>
                </CardHeader>
                <CardContent className="p-8 space-y-3">
                   {quoteForm.defaultTerms.map((term: string, idx: number) => (
                      <div key={idx} className="flex gap-2 items-center group">
                         <Input value={term} onChange={e => updateQuoteTerm(idx, e.target.value)} className="h-10 bg-gray-50 border-none rounded-xl text-xs" />
                         <button onClick={() => removeQuoteTerm(idx)} className="p-2 text-rose-300 hover:text-rose-500 opacity-0 group-hover:opacity-100"><Trash2 size={14}/></button>
                      </div>
                   ))}
                </CardContent>
              </Card>
            ...
            <div className="lg:col-span-4 space-y-6">
              <Card className="border-none shadow-sm bg-white rounded-3xl p-8 border border-gray-100">
                 <CardTitle className="text-sm font-black uppercase tracking-widest text-primary mb-6 flex items-center gap-2"><FileSignature size={18}/> Auth Assets</CardTitle>
                 <ImageUploader label="Default Signature" hint="200x80" initialUrl={quoteForm.signatureUrl} onUpload={url => setQuoteForm({...quoteForm, signatureUrl: url})} aspectRatio="aspect-[2/1]" />
                 <div className="mt-6 pt-6 border-t">
                    <ImageUploader label="Official Seal" hint="200x200" initialUrl={quoteForm.sealUrl} onUpload={url => setQuoteForm({...quoteForm, sealUrl: url})} aspectRatio="aspect-square w-24 mx-auto" />
                 </div>
              </Card>
            </div>
          </div>
        ...
        <TabsContent value="invoice" className="mt-0">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
            <div className="lg:col-span-8 space-y-6">
              <Card className="border-none shadow-sm bg-white rounded-3xl overflow-hidden border border-gray-100">
                <CardHeader className="bg-[#081621] text-white p-8">
                   <CardTitle className="text-lg font-black uppercase tracking-widest flex items-center gap-3"><Printer size={20} className="text-primary" /> Invoice Meta & Branding</CardTitle>
                </CardHeader>
                <CardContent className="p-8 space-y-6">
                   <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div className="space-y-1"><Label className="text-[9px] font-black uppercase">Ref Prefix</Label><Input value={invoiceForm.invoicePrefix} onChange={e => setInvoiceForm({...invoiceForm, invoicePrefix: e.target.value})} className="h-11 bg-gray-50 border-none rounded-xl font-bold" /></div>
                      <div className="space-y-1"><Label className="text-[9px] font-black uppercase">Last Num</Label><Input type="number" value={invoiceForm.invoiceLastNumber} onChange={e => setInvoiceForm({...invoiceForm, invoiceLastNumber: parseInt(e.target.value) || 0})} className="h-11 bg-gray-50 border-none rounded-xl font-black" /></div>
                   </div>
                   <div className="space-y-1"><Label className="text-[9px] font-black uppercase">Header Address (PDF)</Label><Input value={invoiceForm.invoiceHeaderAddress} onChange={e => setInvoiceForm({...invoiceForm, invoiceHeaderAddress: e.target.value})} className="h-11 bg-gray-50 border-none rounded-xl font-bold" /></div>
                   <div className="grid grid-cols-2 gap-4">
                      ...
                      <div className="space-y-1"><Label className="text-[9px] font-black uppercase">Support Email</Label><Input value={invoiceForm.invoiceHeaderEmail} onChange={e => setInvoiceForm({...invoiceForm, invoiceHeaderEmail: e.target.value})} className="h-11 bg-gray-50 border-none rounded-xl" /></div>
                   </div>
                   <div className="space-y-1"><Label className="text-[9px] font-black uppercase">Invoice Tagline</Label><Input value={invoiceForm.invoiceTagline} onChange={e => setInvoiceForm({...invoiceForm, invoiceTagline: e.target.value})} className="h-11 bg-gray-50 border-none rounded-xl italic" /></div>
                   <div className="space-y-1"><Label className="text-[9px] font-black uppercase">Provided Services (Footer List)</Label><Textarea value={invoiceForm.invoiceProvidedServices} onChange={e => setInvoiceForm({...invoiceForm, invoiceProvidedServices: e.target.value})} placeholder="e.g. Home Cleaning, Office Cleaning..." className="h-20 bg-gray-50 border-none rounded-xl" /></div>
                   <div className="space-y-1"><Label className="text-[9px] font-black uppercase">Footer Disclaimer</Label><Input value={invoiceForm.invoiceFooterDisclaimer} onChange={e => setInvoiceForm({...invoiceForm, invoiceFooterDisclaimer: e.target.value})} className="h-11 bg-gray-50 border-none rounded-xl text-gray-400 font-bold" /></div>
                </CardContent>
              </Card>

              <Card className="border-none shadow-sm bg-white rounded-3xl overflow-hidden border border-gray-100">
                <CardHeader className="bg-gray-50/50 p-8 border-b flex flex-row items-center justify-between">
                   <CardTitle className="text-base font-black uppercase tracking-widest text-[#081621] flex items-center gap-3"><ListChecks className="text-primary"/> Default Invoice Terms</CardTitle>
                   <Button type="button" onClick={addInvoiceTerm} variant="outline" size="sm" className="rounded-xl font-black text-[9px] uppercase border-primary/20 text-primary">+ Add Row</Button>
                </CardHeader>
                <CardContent className="p-8 space-y-3">
                   {invoiceForm.invoiceDefaultTerms.map((term: string, idx: number) => (
                      <div key={idx} className="flex gap-2 items-center group">
                         <Input value={term} onChange={e => updateInvoiceTerm(idx, e.target.value)} className="h-10 bg-gray-50 border-none rounded-xl text-xs" />
                         <button onClick={() => removeInvoiceTerm(idx)} className="p-2 text-rose-300 hover:text-rose-500 opacity-0 group-hover:opacity-100"><Trash2 size={14}/></button>
                      </div>
                   ))}
                </CardContent>
              </Card>
            ...
            <div className="lg:col-span-4 space-y-6">
              <Card className="border-none shadow-sm bg-white rounded-3xl p-8 border border-gray-100">
                 <CardTitle className="text-sm font-black uppercase tracking-widest text-primary mb-6 flex items-center gap-2"><FileSignature size={18}/> Auth Signature</CardTitle>
                 <ImageUploader label="Default Signature" hint="200x80" initialUrl={invoiceForm.signatureUrl} onUpload={url => setInvoiceForm({...invoiceForm, signatureUrl: url})} aspectRatio="aspect-[2/1]" />
              </Card>
