...
export default function CreateManualBookingPage() {
  ...
  const totals = useMemo(() => {
    ...
    const finalTotal = isCombinedPricing ? (parseFloat(packagePrice) || 0) : calculatedSubtotal;
    return { calculatedSubtotal, finalTotal };
  }, [items, isCombinedPricing, packagePrice]);
  ...
  const handleCreateBooking = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const bookingData = {
        ...
        address: customer.address,
        serviceTitle: items.length === 1 ? items[0].name : 'Combo Cleaning Package',
        items,
        isCombinedPricing,
        combinedPrice: isCombinedPricing ? totals.finalTotal : null,
        dateTime: schedule.date,
        timeSlot: schedule.time,
        subtotal: isCombinedPricing ? totals.finalTotal : totals.calculatedSubtotal,
    ...
    <div className="space-y-4 pb-24 min-w-0 -mt-6">
      <div className="flex items-center justify-between bg-white p-3 rounded-xl border shadow-sm">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-lg font-black text-gray-900 uppercase tracking-tight leading-none">Service Intake Terminal</h1>
        </div>
              ...
              <Table>
                <TableHeader className="bg-gray-50">
                  <TableRow className="border-none">
                    ...
                    <TableHead className="text-[9px] font-black uppercase text-right w-28">Standard Price</TableHead>
