...
const MOVIE_GROUPS = [
  { id: 'movie_genre', label: 'Genres', desc: 'e.g. Action, Comedy, Horror', icon: Film },
  { id: 'movie_resolution', label: 'Resolutions', desc: 'e.g. 4K, 1080p, 720p', icon: Monitor },
  { id: 'movie_language', label: 'Languages', desc: 'e.g. English, Bengali', icon: Languages }
];

export default function MovieAttributePage() {
  const db = useFirestore();
  const { toast } = useToast();
  const [activeGroup, setActiveGroup] = useState(MOVIE_GROUPS[0].id);
  ...
  const handleAddAttribute = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setNewValue('');
      toast({ title: "Movie Attribute Added" });
    } catch (e) {
  ...
  const handleDelete = async (id: string) => {
    if (!db || !confirm("Delete this movie attribute?")) return;
  ...
  return (
    <div className="space-y-8 pb-24 min-w-0">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 uppercase tracking-tight">Movie Content Attributes</h1>
          <p className="text-muted-foreground text-sm font-medium">Manage specialized metadata for entertainment services</p>
        </div>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        <div className="lg:col-span-4 space-y-4">
          {MOVIE_GROUPS.map((group) => (
            <button
              key={group.id}
            ...
            <CardHeader className="bg-gray-50/50 p-8 border-b flex flex-col sm:flex-row justify-between items-center gap-4">
              <div>
                <CardTitle className="text-lg font-black uppercase tracking-widest text-[#081621]">
                  {MOVIE_GROUPS.find(g => g.id === activeGroup)?.label} Registry
                </CardTitle>
              </div>
