    ...
    <div className="bg-[#F2F4F8] min-h-screen py-4 md:py-8 pb-32 md:pb-16">
      <style jsx global>{`
        @media print {
          ...
          #quote-render-area { shadow: none !important; border-top: none !important; border-radius: 0 !important; }
        }
      `}</style>
      ...
      <div className="container mx-auto px-4 flex flex-col items-center">
        <div className="w-full max-w-[210mm] flex flex-col sm:flex-row justify-between items-center mb-6 gap-4 px-4 no-print">
          ...
          <div className="flex flex-wrap justify-center gap-2">
            <Button variant="outline" onClick={handleWhatsApp} className="rounded-lg gap-2 font-black uppercase text-[9px] h-10 px-4 border-emerald-200 text-emerald-700 bg-emerald-50 hover:bg-emerald-100 transition-all"><MessageCircle size={16} /> WhatsApp</Button>
            <Button className="rounded-lg gap-2 font-black uppercase text-[9px] h-10 px-6 bg-[#1E5F7A] text-white shadow-xl shadow-primary/20 hover:scale-105 transition-all" onClick={() => { setIsDownloading(true); downloadQuotationPDF('quote-render-area', quote.quoteNumber).finally(() => setIsDownloading(false)); }} disabled={isDownloading}>
              {isDownloading ? <Loader2 className="animate-spin h-3 w-3" /> : <Download size={14} />} DOWNLOAD PDF
            </Button>
          </div>
        </div>

        {/* 📄 DOCUMENT CONTENT: OPTIMIZED FOR A4 SINGLE PAGE */}
        <div id="quote-render-area" className="bg-white shadow-2xl relative border-t-[10px] border-[#1E5F7A] rounded-b-[1.5rem]" style={{ width: '210mm', minHeight: '296mm', color: '#333' }}>
          
          {/* Header */}
          <header className="pt-8 px-10 pb-3 flex justify-between items-start border-b-[2px] border-gray-50 mb-6">
            <div className="flex gap-4">
              ...
              <div className="space-y-0.5 text-left">
                ...
                <p className="text-[7px] font-bold text-primary uppercase tracking-widest">Professional Excellence</p>
              </div>
            </div>
            <div className="text-right max-w-[300px]">
              <p className="text-[7px] font-bold text-gray-700 leading-normal uppercase">{headerAddress}</p>
              <p className="text-[7px] font-bold text-[#081621] uppercase mt-0.5">Cell: <span className="font-black">{headerPhone}</span></p>
            </div>
          </header>
          ...
          <div className="px-10 pb-6 space-y-6">
            ...
            <div className="flex justify-between items-start">
              <div className="text-left space-y-3">
                <p className="text-[8px] font-black text-[#1E5F7A] uppercase tracking-[0.2em] border-b border-primary/20 pb-0.5 w-fit">Recipient Profile</p>
                <div className="space-y-0.5">
                  <h4 className="text-base font-black text-[#081621] uppercase tracking-tight">{quote.customerInfo?.name}</h4>
                  <p className="text-[9px] font-bold text-gray-600">{quote.customerInfo?.phone}</p>
                  <p className="text-[8px] text-gray-500 font-medium leading-relaxed max-w-[350px] uppercase italic">{quote.customerInfo?.address}</p>
                </div>
              </div>
              <div className="text-right space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <p className="text-[9px] font-black text-[#081621]">{quote.issueDate ? format(new Date(quote.issueDate), 'dd MMM yyyy') : 'N/A'}</p>
                  </div>
                  <div>
                    <p className="text-[8px] font-black text-gray-400 uppercase tracking-widest">Valid Until</p>
                    <p className="text-[9px] font-black text-rose-600">{quote.expiryDate ? format(new Date(quote.expiryDate), 'dd MMM yyyy') : 'N/A'}</p>
                  </div>
                </div>
              </div>
            </div>

            <div className="overflow-hidden border border-[#081621] rounded-xl shadow-sm">
              <table className="w-full border-collapse text-[9px]">
                <thead className="bg-[#081621] text-white">
                  <tr>
                    <th className="py-2.5 px-3 font-black uppercase text-left w-10">SL</th>
                    <th className="py-2.5 px-3 font-black uppercase text-left">Service Components</th>
                    <th className="py-2.5 px-3 font-black uppercase text-center w-24">Unit/Area</th>
                    <th className="py-2.5 px-3 font-black uppercase text-right w-24">Unit Price</th>
                    <th className="py-2.5 px-3 font-black uppercase text-right w-28">Subtotal</th>
                  </tr>
                </thead>
                <tbody className="font-bold bg-white">
                  {quote.items?.map((item: any, i: number) => (
                    <tr key={i} className="border-t border-gray-100 align-top">
                      <td className="py-2.5 px-3 text-left text-gray-400">{i + 1}</td>
                      <td className="py-2.5 px-3 text-left">
                        <p className="font-black text-gray-900 uppercase leading-tight mb-0.5">{item.name}</p>
                        <p className="text-[7.5px] text-gray-500 font-medium leading-relaxed italic">{item.description}</p>
                      </td>
                      <td className="py-2.5 px-3 text-center text-gray-600 uppercase font-black">{item.quantity} {item.unit || 'Qty'}</td>
                      <td className="py-2.5 px-3 text-right text-gray-600">৳{item.price?.toLocaleString()}</td>
                      <td className="py-2.5 px-3 text-right text-[#081621] font-black">৳{(item.price * item.quantity).toLocaleString()}</td>
                    </tr>
                  ))}
                  <tr className="border-t-2 border-[#081621] bg-gray-50/50">
                    <td colSpan={4} className="py-2 px-8 text-right font-black uppercase text-[8px] tracking-widest">Base Estimate Total</td>
                    <td className="py-2 px-3 text-right font-black text-[10px]">৳{quote.subtotal?.toLocaleString()}/-</td>
                  </tr>
                  <tr className="border-t border-[#081621] bg-[#1E5F7A] text-white">
                    ...
                    <td className="py-3 px-3 text-right font-black text-base">৳{quote.total?.toLocaleString()}/-</td>
                  </tr>
                </tbody>
              </table>
            </div>

            <div className="p-3 bg-gray-50 rounded-xl border border-gray-100 flex flex-col gap-0.5 text-left">
              <p className="text-[6px] font-black uppercase text-gray-400 tracking-[0.3em]">Value Proof (In words):</p>
              <p className="text-[9px] font-black text-[#081621] italic">"{numberToWords(parseFloat(quote.total) || 0)}"</p>
            </div>

            <div className="space-y-2">
               <h5 className="text-[9px] font-black uppercase tracking-widest text-primary border-b border-primary/20 pb-0.5 w-fit">General Terms & Conditions</h5>
               <div className="bg-white p-5 rounded-2xl border border-gray-100 shadow-inner">
                  <div className="space-y-1.5">
                    {terms.map((term: string, i: number) => (
                      <div key={i} className="flex gap-2 items-start">
                        <span className="text-[9px] font-black text-primary min-w-[15px]">{i + 1}.</span>
                        <p className="text-[9.5px] font-medium text-gray-600 leading-normal">{term}</p>
                      </div>
                    ))}
                  </div>
               </div>
            ...
            <div className="avoid-break grid grid-cols-2 gap-32 items-end pt-4 pb-4">
              <div className="text-center space-y-3">
                <div className="border-b-[2px] border-gray-100 h-8"></div>
              ...
              <div className="flex flex-col items-center justify-end text-center space-y-3">
                <div className="h-14 w-28 relative border-b-[2px] border-primary/10 flex items-center justify-center">
                   {signatureUrl ? <Image src={signatureUrl} alt="Sign" fill className="object-contain" unoptimized /> : <Badge variant="outline" className="text-[7px] font-black border-dashed border-primary/30 text-primary uppercase">Authorized Digitally</Badge>}
                </div>
            ...
            <div className="pt-4 border-t border-gray-100">
               <div className="text-center space-y-1 mb-4">
                  <p className="text-[11px] font-black text-primary flex items-center justify-center gap-2 uppercase tracking-widest">{tagline} <Star size={8} fill="currentColor"/></p>
                  <p className="text-[7px] text-gray-400 font-bold uppercase tracking-[0.2em]">Our Professional Service Network</p>
               </div>
                    ...
                    <div key={colIdx} className="space-y-1">
                       {providedServices.filter((_: string, i: number) => i % 3 === colIdx).map((service: string, sIdx: number) => (
                         <div key={sIdx} className="flex items-center gap-1.5">
                            ...
                            <span className="text-[8px] font-bold text-gray-500 uppercase truncate">{service}</span>
                         </div>
                       ))}
                    </div>
                  ))}
               ...
               <p className="text-[7px] text-gray-300 font-bold uppercase text-center mt-6 tracking-[0.3em]">{footerDisclaimer}</p>
            </div>
          </div>
        </div>
      </div>
