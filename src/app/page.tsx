  ...
  return (
    <PublicLayout>
      <div className="flex flex-col bg-[#F2F4F8] min-h-screen">
        ...
        {/* HERO SECTION - Daraz Style Slider */}
          ...
          <div className="container mx-auto px-0 lg:px-4">
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-4">
              ...
              <div className={cn("col-span-1", (sideBanners.length > 0) ? "lg:col-span-9" : "lg:col-span-12")}>
                <div className="relative aspect-[21/9] md:aspect-[982/400] w-full lg:rounded-xl overflow-hidden shadow-sm bg-gray-100">
                  {bannersLoading ? (
                        ...
                        {mainBanners.map((banner) => (
                          <CarouselItem key={banner.id} className="h-full min-w-0 shrink-0 grow-0 basis-full relative pl-0">
                            <Link href={banner.buttonLink || '#'} className="block w-full h-full relative">
                              ...
                              {/* Content Overlay */}
                              <div className="absolute inset-0 bg-gradient-to-r from-black/40 to-transparent flex flex-col justify-center p-6 md:p-12">
                                <h2 className="text-white text-xl md:text-4xl font-black uppercase tracking-tight drop-shadow-lg mb-2">{banner.title}</h2>
                                <p className="text-white/90 text-xs md:text-lg font-medium drop-shadow-md mb-4 max-w-md line-clamp-2">{banner.subtitle}</p>
                                {banner.isButtonEnabled !== false && (
                                  <Button size="sm" className="w-fit rounded-full px-6 font-black uppercase text-[10px] md:text-xs" style={{ backgroundColor: banner.buttonColor }}>
        ...
        {/* CATEGORY GRID (4 Columns Mobile / 8 Columns Desktop) */}
          ...
          <div className="bg-white rounded-xl shadow-sm p-4 md:p-6">
            <div className="grid grid-cols-4 md:grid-cols-8 gap-4 md:gap-8">
              {allTopNav?.length ? allTopNav.sort((a,b) => (a.order||0)-(b.order||0)).map((cat) => (
                ...
                >
                  <div className="w-12 h-12 md:w-16 md:h-16 rounded-2xl bg-gray-50 flex items-center justify-center p-2 transition-all group-hover:bg-primary/10 group-hover:scale-110">
                    <LayoutGrid size={24} className="text-gray-400 group-hover:text-primary" />
                  </div>
                  <span className="text-[10px] md:text-xs font-bold text-center text-gray-700 group-hover:text-primary transition-colors line-clamp-1 truncate w-full px-1">
                    {cat.name}
                  </span>
                </Link>
              )) : Array(8).fill(0).map((_, i) => (
                <div key={i} className="flex flex-col items-center gap-2">
                  <div className="w-12 h-12 md:w-16 md:h-16 rounded-2xl bg-gray-50 animate-pulse" />
        ...
        <section className="container mx-auto px-4 py-2">
          <div className="bg-white rounded-xl shadow-sm overflow-hidden">
            <div className="bg-[#fcd34d]/10 border-b border-[#fcd34d]/20 p-4 flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className="bg-[#fcd34d] p-1.5 rounded-lg text-[#92400e]"><Flashlight size={18} fill="currentColor" /></div>
                <h2 className="text-sm md:text-lg font-black uppercase tracking-tight text-[#92400e]">Special Offers</h2>
                <div className="hidden md:flex items-center gap-2 ml-4">
                  <span className="text-[10px] font-bold text-[#92400e] uppercase">Ending in:</span>
                  <div className="flex gap-1">
                    {['02', '14', '55'].map((t, i) => (
                      <span key={i} className="bg-[#92400e] text-white text-[10px] font-black px-1.5 py-0.5 rounded">
                        {t}
                      </span>
                    ))}
                  </div>
              ...
              <Button variant="link" className="text-[#92400e] font-black uppercase text-[10px] p-0 h-auto" asChild>
                <Link href="/products">Shop More <ChevronRight size={14} /></Link>
              </Button>
            </div>
            <div className="p-4 overflow-x-auto no-scrollbar">
              <div className="flex gap-4 min-w-max pb-2">
                {featuredProducts.length > 0 ? featuredProducts.map((product) => (
                  <div key={product.id} className="w-[140px] md:w-[180px] shrink-0">
                    <ProductCard product={product} />
                  </div>
                )) : Array(5).fill(0).map((_, i) => (
                  <div key={i} className="w-[140px] md:w-[180px] h-[220px] bg-gray-50 animate-pulse rounded-xl" />
                ))}
              </div>
            </div>
          </div>
        ...
        {/* JUST FOR YOU - Main Product Grid (2 Columns Mobile) */}
        <section className="container mx-auto px-4 py-8">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-lg md:text-2xl font-black uppercase tracking-tight text-gray-900 flex items-center gap-2">
              <Sparkles className="text-primary" size={20} fill="currentColor" /> Just For You
            </h2>
          </div>
          
          {productsLoading ? (
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3 md:gap-4">
              {Array(12).fill(0).map((_, i) => <div key={i} className="aspect-[3/4] bg-white rounded-xl animate-pulse" />)}
            </div>
          ) : (
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3 md:gap-4">
          ...
          {!latestProducts.length && !productsLoading && (
            <div className="py-20 text-center bg-white rounded-2xl border-2 border-dashed">
              <Package size={48} className="mx-auto text-gray-200 mb-4" />
              <p className="text-xs font-bold text-gray-400 uppercase">No products available at the moment</p>
            </div>
          )}
        </section>
        ...
        <section className="container mx-auto px-4 py-8 border-t border-gray-200 mt-8">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-lg md:text-2xl font-black uppercase tracking-tight text-gray-900 flex items-center gap-2">
              <Wrench className="text-primary" size={20} /> Essential Services
            </h2>
            <Button variant="outline" size="sm" className="rounded-full font-black text-[10px] uppercase h-8" asChild>
            ...
            {activeServices.map((service) => (
              <Link key={service.id} href={`/service/${service.id}`} className="group relative aspect-[4/3] rounded-xl overflow-hidden shadow-sm bg-white border border-gray-100 block">
                {service.imageUrl ? (
                  ...
                  <div className="w-full h-full bg-primary/5 flex items-center justify-center text-primary/40"><Wrench size={32} /></div>
                )}
                <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent flex flex-col justify-end p-3">
                  <h3 className="text-white font-black uppercase text-[10px] md:text-xs leading-tight line-clamp-1">{service.title}</h3>
