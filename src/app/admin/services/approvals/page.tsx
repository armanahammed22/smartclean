...
export default function ServiceApprovalsPage() {
  ...
  const pendingQuery = useMemoFirebase(() => {
    if (!db) return null;
    return query(collection(db, 'services'), where('status', '==', 'Pending'), orderBy('createdAt', 'desc'));
  }, [db]);

  const { data: pendingServices, isLoading } = useCollection(pendingQuery);
    ...
    <div className="space-y-8 pb-20">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <Card className="border-none shadow-sm bg-orange-50 text-orange-700">
          <CardContent className="p-6 flex items-center justify-between">
            <div>
              ...
              <h3 className="text-3xl font-black">{pendingServices?.length || 0}</h3>
            </div>
        ...
        <Card className="border-none shadow-sm bg-indigo-50 text-indigo-700">
          <CardContent className="p-6 flex items-center justify-between">
            <div>
              <p className="text-indigo-700/80 text-[10px] font-black uppercase tracking-widest leading-none mb-1">Submission Rate</p>
        ...
        <Card className="border-none shadow-sm bg-[#081621] text-white">
          <CardContent className="p-6 flex items-center justify-between">
            <div>
              <p className="text-white/60 text-[10px] font-black uppercase tracking-widest leading-none mb-1">Moderator Score</p>
              <h3 className="text-3xl font-black">99%</h3>
            </div>
            <CheckCircle2 size={40} className="opacity-20 text-primary" />
          </CardContent>
        </Card>
      </div>
        ...
        <CardContent className="p-0 overflow-x-auto">
          <Table>
            <TableHeader className="bg-gray-50/50">
              <TableRow>
                <TableHead className="font-bold py-5 pl-8">Service Preview</TableHead>
                <TableHead className="font-bold">Provider Info</TableHead>
                <TableHead className="font-bold">Category</TableHead>
                <TableHead className="font-bold">Price Start</TableHead>
            ...
            <TableBody>
              ...
              {!pendingServices?.length && !isLoading && (
                <TableRow>
