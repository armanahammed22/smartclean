        ...
        <div className="grid lg:grid-cols-5 h-full">
          ...
          <div className="lg:col-span-3 p-6 md:p-8 bg-white">
            <DialogHeader className="mb-6">
              <DialogTitle className="text-2xl font-bold font-headline">{t('checkout_title')}</DialogTitle>
              <DialogDescription>{t('delivery_info')}</DialogDescription>
            </DialogHeader>
              ...
              <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel className="text-xs font-bold uppercase tracking-wider">{t('full_name')}</FormLabel>
                        <FormControl>
                          <Input placeholder="John Doe" {...field} className="h-11 bg-muted/30" />
                        </FormControl>
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel className="text-xs font-bold uppercase tracking-wider">{t('phone_number')}</FormLabel>
                        <FormControl>
                          <Input placeholder="+880 1XXX-XXXXXX" {...field} className="h-11 bg-muted/30" />
                        </FormControl>
                        <FormMessage className="text-[10px]" />
                      </FormItem>
                    )}
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel className="text-xs font-bold uppercase tracking-wider">{t('delivery_address')}</FormLabel>
                      <FormControl>
                        <Textarea 
                ...
                {hasServices && (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 p-4 bg-primary/5 rounded-xl border border-primary/20">
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem className="flex flex-col">
                          <FormLabel className="text-xs font-bold uppercase tracking-wider">{t('booking_date')}</FormLabel>
                          <Popover>
                            <PopoverTrigger asChild>
                              <FormControl>
                                <Button
                                  variant={"outline"}
                                  className={cn(
                                    "h-11 w-full pl-3 text-left font-normal bg-white",
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <FormLabel className="text-xs font-bold uppercase tracking-wider">{t('booking_time')}</FormLabel>
                ...
                <div className="space-y-3">
                  <FormLabel className="text-xs font-bold uppercase tracking-wider">{t('payment_method')}</FormLabel>
                  <FormField
                    control={form.control}
                    name="paymentMethod"
                    render={({ field }) => (
                      ...
                      >
                        <div className="flex items-center space-x-2 rounded-lg border p-3 hover:bg-muted/50 cursor-pointer has-[:checked]:border-primary has-[:checked]:bg-primary/5">
                          <RadioGroupItem value="cod" id="cod" disabled={hasServices} />
                          <label htmlFor="cod" className="text-xs font-bold flex items-center gap-2 cursor-pointer">
                            <Wallet size={14} /> {t('payment_cod')}
                          </label>
                        </div>
                        <div className="flex items-center space-x-2 rounded-lg border p-3 hover:bg-muted/50 cursor-pointer has-[:checked]:border-primary has-[:checked]:bg-primary/5">
                          <RadioGroupItem value="cash_in_hand" id="cash_in_hand" />
                          <label htmlFor="cash_in_hand" className="text-xs font-bold flex items-center gap-2 cursor-pointer">
                            <Smartphone size={14} /> {t('payment_cash_hand')}
                          </label>
                        </div>
                        <div className="flex items-center space-x-2 rounded-lg border p-3 hover:bg-muted/50 cursor-pointer has-[:checked]:border-primary has-[:checked]:bg-primary/5">
                          <RadioGroupItem value="bkash" id="bkash" />
                          <label htmlFor="bkash" className="text-xs font-bold flex items-center gap-2 cursor-pointer">
                            bKash / Nagad
                          </label>
                        </div>
                        <div className="flex items-center space-x-2 rounded-lg border p-3 hover:bg-muted/50 cursor-pointer has-[:checked]:border-primary has-[:checked]:bg-primary/5">
                          <RadioGroupItem value="card" id="card" />
                          <label htmlFor="card" className="text-xs font-bold flex items-center gap-2 cursor-pointer">
                            <CreditCard size={14} /> Card (SSL)
                          </label>
                        </div>
                      </RadioGroup>
                    )}
                ...
                <Button type="submit" className="w-full h-12 font-bold rounded-xl shadow-lg mt-4" disabled={isSubmitting || items.length === 0}>
          ...
          {/* Cart Side */}
          <div className="lg:col-span-2 bg-[#F9FAFB] p-6 border-l">
