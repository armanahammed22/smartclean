  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC]">
      ...
      <div className="container mx-auto px-4 md:px-8 py-8">
        <div className="flex flex-col lg:grid lg:grid-cols-10 gap-8 items-start">
          
          {/* 🖼️ LEFT COLUMN: STICKY MEDIA PANEL (30%) */}
              ...
              <CardContent className="p-5 space-y-6">
                ...
                <div className="space-y-3">
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    ...
                    <div className="flex items-center justify-center border-2 border-dashed border-gray-200 rounded-lg aspect-square bg-gray-50/50 hover:bg-white hover:border-primary/50 transition-all">
                ...
                <div className="space-y-2 pt-4 border-t border-gray-50">
                  ...
                  <div className="relative">
                    <Wrench size={12} className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" />
                    <Input value={formData.videoUrl} onChange={e => setFormData({...formData, videoUrl: e.target.value})} placeholder="YouTube URL" className="h-10 pl-9 bg-gray-50 border-none rounded-xl text-[10px] font-mono shadow-inner" />
                  </div>
                </div>
              </CardContent>
            </Card>
          ...
          <main className="lg:col-span-7 space-y-6">
            ...
            <Card className="border-none shadow-sm rounded-[18px] bg-white border border-gray-100">
              <CardHeader className="bg-gray-50/50 p-6 border-b"><CardTitle className="text-[11px] font-black uppercase tracking-widest">Basic Information & Pricing</CardTitle></CardHeader>
              <CardContent className="p-6 md:p-8 space-y-6">
                ...
                <div className="space-y-2 pt-4 border-t border-gray-50">
                  <Label className="text-[10px] font-black uppercase text-muted-foreground ml-1">Service Narrative (Description)</Label>
            ...
            {/* SECTION 2: ADD-ONS (COMPACT SELECTOR) */}
            <Card className="border-none shadow-sm rounded-[18px] bg-white border border-gray-100">
              <CardHeader className="bg-gray-50/50 p-5 border-b flex flex-row items-center justify-between">
                <div>
                  <CardTitle className="text-[11px] font-black uppercase tracking-widest">Optional Add-ons</CardTitle>
                </div>
              ...
              <CardContent className="p-6 md:p-8 space-y-4">
                <div className="relative group">
                  ...
                  <Input 
                    ...
                    placeholder="Search sub-services repository..." 
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <Card className="border-none shadow-sm rounded-[18px] bg-white border border-gray-100">
                <CardHeader className="bg-emerald-50/30 p-5 border-b flex flex-row items-center justify-between">
                  <CardTitle className="text-[10px] font-black uppercase text-emerald-700 tracking-widest flex items-center gap-2"><Check size={14}/> Included List</CardTitle>
              ...
              <Card className="border-none shadow-sm rounded-[18px] bg-white border border-gray-100">
                <CardHeader className="bg-rose-50/30 p-5 border-b flex flex-row items-center justify-between">
                  <CardTitle className="text-[10px] font-black uppercase text-rose-700 tracking-widest flex items-center gap-2"><X size={14}/> Exclusion List</CardTitle>
            ...
            <Card className="border-none shadow-sm rounded-[18px] bg-white border border-gray-100">
              <CardHeader className="bg-indigo-50/30 p-5 border-b flex flex-row items-center justify-between">
                <CardTitle className="text-[10px] font-black uppercase text-indigo-700 tracking-widest flex items-center gap-2"><Zap size={14}/> Marketing Highlights</CardTitle>
                <button type="button" onClick={() => addArrayItem('features', { icon: 'Zap', title: 'New Feature', desc: '' })} className="text-indigo-600 hover:scale-110 transition-transform"><Plus size={16}/></button>
              </CardHeader>
              <CardContent className="p-5 md:p-8">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {formData.features?.map((f: any, i: number) => (
                    <div key={i} className="p-4 bg-gray-50 rounded-2xl border border-gray-100 space-y-3 relative group animate-in zoom-in-95">
                      <div className="grid grid-cols-2 gap-3">
                        <Input value={f.icon} onChange={e => updateArrayItem('features', i, { ...f, icon: e.target.value })} placeholder="Icon Key" className="h-9 bg-white border-none rounded-lg text-[10px] font-black uppercase shadow-inner" />
                        <Input value={f.title} onChange={e => updateArrayItem('features', i, { ...f, title: e.target.value })} placeholder="Highlight Title" className="h-9 bg-white border-none rounded-lg font-black text-[10px] uppercase shadow-inner" />
                      </div>
                      <Input value={f.desc} onChange={e => updateArrayItem('features', i, { ...f, desc: e.target.value })} placeholder="Teaser text..." className="h-9 bg-white border-none rounded-lg text-[10px] shadow-inner" />
                      <button type="button" onClick={() => removeArrayItem('features', i)} className="absolute -top-2 -right-2 bg-rose-100 text-rose-600 p-1.5 rounded-full opacity-0 group-hover:opacity-100 transition-opacity shadow-sm"><X size={12}/></button>
                    </div>
                  ))}
                </div>
              </CardContent>
            ...
            <div className="pt-8 flex flex-col md:flex-row items-center justify-end gap-4 border-t border-gray-100">
              <Button variant="ghost" onClick={() => router.push('/admin/services')} className="w-full md:w-auto h-12 px-8 rounded-xl font-bold uppercase text-[10px] tracking-widest text-gray-400">Cancel & Discard</Button>
              <div className="flex gap-3 w-full md:w-auto">
                <Button variant="outline" onClick={() => handleSave('Inactive')} disabled={isSaving} className="flex-1 md:flex-none h-12 px-8 rounded-xl font-black uppercase text-[10px] tracking-widest border-gray-200">Save Draft</Button>
                <Button onClick={() => handleSave()} disabled={isSaving} className="flex-1 md:flex-none h-12 px-12 rounded-xl font-black bg-primary text-white shadow-xl shadow-primary/30 uppercase tracking-widest text-[10px] active:scale-95 transition-all">
                  {isSaving ? <Loader2 className="animate-spin" /> : <><ShieldCheck size={16} className="mr-2" /> Publish Logic</>}
                </Button>
              </div>
            </div>
